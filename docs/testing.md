...
| `RateLimiterIntegrationTest` | `RateLimiter`'s fixed-window counting against real Redis: allows exactly up to capacity then rejects, and - the property Phase 15's per-email login limiter depends on - two different keys are counted completely independently, so one identity being throttled has no effect on another's remaining capacity. |
